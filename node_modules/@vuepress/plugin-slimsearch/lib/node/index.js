...
`)},N=async(e,t,a,r,i)=>{const o=C(i),n=e.pages.find(({filePathRelative:s})=>s?.toLowerCase()===o.toLowerCase());if(n){const s=L(n,r,t.customFields,t.indexContent),{pathLocale:d}=n,c=r.addPath(n.path).toString(),u=a[d];Array.from(u._documentIds.values()).filter(y=>y.startsWith(c)).forEach(y=>{E(u,y)}),M(u,s),await z(u);const h=`export default ${JSON.stringify(JSON.stringify(u))}
`;await e.writeTemp(`slimsearch/${g(d)}.js`,h)}},oe=async(e,t,a,r)=>{const i=C(r),o=e.pages.find(({filePathRelative:n})=>n?.toLowerCase()===i.toLowerCase());if(o){const{pathLocale:n}=o,s=a.addPath(o.path).toString(),d=t[n];Array.from(d._documentIds.values()).filter(u=>u.startsWith(s)).forEach(u=>{E(d,u)}),await z(d);const c=`export default ${JSON.stringify(JSON.stringify(d))}
`;await e.writeTemp(`slimsearch/${g(n)}.js`,c)}},ce=(e={})=>t=>{t.env.isDebug&&Q.info("Options:",e);const a=new re;let r=null;return{name:w,define:{__SLIMSEARCH_SUGGESTION__:e.suggestion??!0,__SLIMSEARCH_CUSTOM_FIELDS__:T(e.customFields?.map(({formatter:i},o)=>i?[o.toString(),i]:null).filter(i=>i!==null)??[]),__SLIMSEARCH_LOCALES__:I({app:t,name:w,config:e.locales,default:ie}),__SLIMSEARCH_OPTIONS__:{searchDelay:e.searchDelay??150,suggestDelay:e.suggestDelay??0,queryHistoryCount:e.queryHistoryCount??5,resultHistoryCount:e.resultHistoryCount??5,hotKeys:e.hotKeys??[{key:"k",ctrl:!0},{key:"/",ctrl:!0}],worker:e.worker??"slimsearch.worker.js"}},clientConfigFile:`${ee}config.js`,extendsBundlerOptions:i=>{P(i,t,"slimsearch",!0),j(i,t,["@vuepress/helper","fflate","vuepress-shared"])},onInitialized:async()=>{r=await Y(t,e,a)},onPrepared:async()=>{const{isBuild:i,isDev:o}=t.env;await Promise.all([o?le(t,r):null,se(t,a),o?ne(t,e):null]),i&&a.clear()},onWatched:(i,o)=>{if(e.hotReload??t.env.isDebug){const n=F("pages",{cwd:t.dir.temp(),ignoreInitial:!0,ignored:(s,d)=>!!(d?.isFile()&&!s.endsWith(".vue"))});n.on("add",s=>{N(t,e,r,a,s)}),n.on("change",s=>{N(t,e,r,a,s)}),n.on("unlink",s=>{oe(t,r,a,s)}),o.push(n)}},onGenerated:()=>ae(t,e,r)}};var b=(e=>(e.Heading="h",e.Anchor="a",e.Text="t",e.CustomFields="c",e))(b||{});export{b as IndexField,ce as slimsearchPlugin};
